...
void Bee::move()
{
    ...
    if (dir == UP && m_row > 1) m_row--;
    else if (dir == DOWN && m_row < m_room->rows()) m_row++;
    else if (dir == LEFT && m_col > 1) m_col--;
    else if (dir == RIGHT && m_col < m_room->cols()) m_col++;
    // ATTEMPTED - TODO:  Attempt to move in direction dir; if bee can't move, don't move.
}

bool Bee::getSwatted(int dir)  // return true if dies
{
    // ATTMPTED - TODO:  If the bee has been swatted once before, return true
...
{
    m_age++;
    if (dir == UP && m_room->numBeesAt(m_row - 1, m_col) > 0) m_room->swatBeeAt(m_row - 1, m_col, dir);
    else if (dir == DOWN && m_room->numBeesAt(m_row + 1, m_col) > 0) m_room->swatBeeAt(m_row + 1, m_col, dir);
    else if (dir == LEFT && m_room->numBeesAt(m_row, m_col - 1) > 0) m_room->swatBeeAt(m_row, m_col - 1, dir);
    else if (dir == RIGHT && m_room->numBeesAt(m_row, m_col + 1) > 0) m_room->swatBeeAt(m_row, m_col + 1, dir);
    else {
...
Room::~Room()
{
    // ATTEMPED - TODO:  Delete the player and all remaining dynamically allocated bees.
...
int Room::numBeesAt(int r, int c) const
{
    ...
    //ATTEMPTED - TODO:  Return the number of bees at row r, column c.
    return num;
}
...
void Room::display() const
{
    ...
    // ATTEMPTED - TODO:  If one bee is at some grid point, set the char to 'B'.
    //        If it's 2 though 8, set it to '2' through '8'.
    //        For 9 or more, set it to '9'.
    for (int i = 0; i < m_nBees; i++) {
        int rowVal = m_bees[i]->row() - 1;
        int colVal = m_bees[i]->col() - 1;
        if (grid[rowVal][colVal] == '.') grid[rowVal][colVal] = 'B';
        else if (grid[rowVal][colVal] == 'B') grid[rowVal][colVal] = '2';
        else if (grid[rowVal][colVal] < '9') grid[rowVal][colVal]++;
    }
...
bool Room::addBee(int r, int c)
{
    if (m_nBees == MAXBEES) return false;
        ...
        Bee* newBee = new Bee(this, r, c);
        m_bees[m_nBees] = newBee;
        m_nBees++;
        return true;
    }
